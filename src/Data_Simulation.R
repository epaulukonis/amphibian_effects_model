...
library(DescTools)
library(reshape2)
setwd('C:/Users/epauluko/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Paulukonis_Documents/manuscript_pop_modeling/amphibian_effects_model')


mean(BMDS_headline_fin$Effect)
...
bw_sim<-replicate(nsims,
                  {normalize <- lapply(by_s, function(y) rnorm(nrow(y), mean=y[[11]], sd=y[[12]]))
...
write.csv(BMDS_headline_fin,'data_out/BMDS_headline_fin_61421.csv') #save whole file for bmds analysis
