...
names(effects)
df_sig<-effects[,c(1,5,8,10,18)] #make sure this output matches
...
#change format to matric to run CA trend test to test for trends in increasing significance
ch0=list()
...
bw_sim<-replicate(nsims,
                  {normalize <- lapply(by_s, function(y) rnorm(nrow(y), mean=y[[11]], sd=y[[21]]))
...
BMDS_Run4<-cbind(exp_n,mort_n,derm_dose) #name the output whatever run you'd like
BMDS_Run4<-BMDS_Run4[,c(1,6,2,4)]
...
write.csv(BMDS_Run4,'data_out/BMDS_Run4.csv') #save whole file for bmds analysis
