...
This script creates an SSD using LC10 toxicity data derived from the ECOTOX extract for Glyphosate for amphibians.
...
# Fit 4 distributions to the glyphosate surrogate data to see which have best-fit
...
for(i in single.ssd.chem) {
  ...
  IndividualSSDs <- LC10 %>% filter(Chemical == i) %>%
    group_by(Species) %>%
    summarise(
...
gly.LC10.HC.data <- HC.list

# CHange column names
colnames(gly.LC10.HC.data) <- c("Percentile", "HC.Value", "HC.Upper.CI", "HC.Lower.CI")

# Add in distribution and data ID
gly.LC10.HC.data$Distribution <- "llogis"
gly.LC10.HC.data$Data.ID <- "Glyphosate.LC10"
```

We can now save this as an Excel sheet or CSV for use later. 

# Plot the SSD for the Glyphosate LC10 data
```{r}
# Filter the data to only include LC10 values for glyphosate
gly.lc10.data <- LC10 %>%
...
gly.lc10.data <- unique(gly.lc10.data)
...
## Generate x values
newxs <- 10^(seq(log10(0.01), (log10(max(df$geomean))+1), length.out = 1000)) #creating a sequence from the smallest value to the largest value (with a bit extra so the plot does not just end at the largest value)
...
gly.lc10.plot <- ggplot()+
  ...
  labs(x = expression(paste('Glyphosate (LC10) [ mg ', L^-1, ' ]')), 
...
```{r}
# Glyphosate LC10 data saving
write.csv(gly.LC10.HC.data, file = "Glyphosate_LC10_Data.csv")

# Save the surrogate SSD image
tiff("Glyphosate_LC10_SSD_Image.tiff", units = "in", width = 7.5, height = 7, res = 300) # load an empty tiff file with these dimensions
gly.lc10.plot # put in the final results plot
