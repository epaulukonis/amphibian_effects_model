...
This script creates an SSD using LC01 toxicity data derived from the ECOTOX extract for Glyphosate for amphibians.

# Set working directory
```{r}
# Set working directory to file location
setwd("C:/Users/adhond/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Tom Purucker Work/pyra-glypho-ssds")
```
...
data <- readxl::read_xlsx("Glyphosate_ECOTOX_Extract_12-08.xlsx", sheet = 1)
```

# Clean up data and filter for SSD model
Extract relevant columns, rename them, remove NAs, and filter only Glyphosate 
```{r}
# Extract only relevant columns
data <- data[, c(1:4,20, 25, 28)]

# Rename columns
colnames(data) <- c("CAS", "Chemical", "Species", "Common Name", "Conc", "Conc Units", "Endpoint")

# Remove NAs
data <- na.omit(data)

# Rename the chemicals into one single chem (all glyphosate)
data$Chemical[data$Chemical == "N-(Phosphonomethyl)glycine compd. with 2-propanamine (1:1)"] <- "Glyphosate"
data$Chemical[data$Chemical == "N-(Phosphonomethyl)glycine potassium salt (1:1)"] <- "Glyphosate"
data$Chemical[data$Chemical == "N-(Phosphonomethyl)glycine sodium salt (2:3)"] <- "Glyphosate"
```

# Filter by endpoint
```{r}
# Filter by LC05s first
LC05 <- data %>%
  filter(
    Endpoint == "LC05"
  )

# Filter by LC01s as well
LC01 <- data %>%
  filter(
    Endpoint == "LC01"
  )
...
So, there is a lot of data for LC10 values for amphibians, only 1 species for LC05, and a few species for LC01.

# Fit 4 distributions to the glyphosate data to see which have best-fit
...
for(i in single.ssd.chem) {
  ...
  IndividualSSDs <- LC01 %>% filter(Chemical == i) %>%
    group_by(Species) %>%
    summarise(
...
# Save as LC01 HC data. Add in distribution used, as well as what data this is coming from
gly.LC01.HC.data <- HC.list

# CHange column names
colnames(gly.LC01.HC.data) <- c("Percentile", "HC.Value", "HC.Upper.CI", "HC.Lower.CI")

# Add in distribution and data ID
gly.LC01.HC.data$Distribution <- "llogis"
gly.LC01.HC.data$Data.ID <- "Glyphosate.LC01"

```

We can now save this as an Excel sheet or CSV for use later. 

# Plot the SSD for the Glyphosate LC01 data
```{r}
# Filter the data to only include LC01 data for glyphosate
gly.LC01.data <- LC01 %>%
  group_by(Species) %>% # group by species
  summarise(
    geomean = geoMean(Conc)
  )

# Remove duplicate values as we have already calculated the geomean
gly.LC01.data <- unique(gly.LC01.data)
...
ggplot(data = df) + # the input data is the glyphosate data
  ...
  geom_text(aes(x = geomean, y = frac, label = Species), hjust = 1.1, size = 3) + # the text is the species label
  theme_bw() +
  scale_x_log10(limits = c(0.00005, max(df$geomean))) + # set the x axis limits
  labs(x = expression(paste('Concentration of Glyphosate (LC01) [ mg ', L^-1, ' ]')), 
...
## Generate x values
newxs <- 10^(seq(log10(0.1), (log10(max(df$geomean))+0.5), length.out = 1000)) #creating a sequence from the smallest value to the largest value (with a bit extra so the plot does not just end at the largest value)
...
gly.lc01.plot <- ggplot()+
  ...
  geom_text(data = df, aes(x = fit, y = frac, label = Species), hjust = 1.2, size = 3)+
  theme_bw() + # changes plot theme
  scale_x_log10() + # log scale on x axis
  labs(x = expression(paste('Glyphosate (LC01) [ mg ', L^-1, ' ]')), 
...
```{r}
# Glyphosate LC01 data saving
write.csv(gly.LC01.HC.data, file = "Glyphosate_LC01_Data.csv")

# Save the LC01 SSD image
tiff("Glyphosate_LC01_SSD_Image.tiff", units = "in", width = 7.5, height = 7, res = 300) # load an empty tiff file with these dimensions
gly.lc01.plot # put in the final results plot
